...
ALTER TABLE tbl_book_rent -- N(다,多) Table
ADD CONSTRAINT fk_buyer -- FK 이름
FOREIGN KEY(br_bcode) -- N table의 칼럼
REFERENCES tbl_buyer(bu_code); -- 1 table과 칼럼
...
-- 기존에 사용하던 SEQ를 삭제하고 다시 생성할 경우 먼저
-- 반드시 적용되는 Table의 칼럼 값을 확인해야 한다
-- seq 칼럼의 최대값을 확인하고
SELECT MAX(br_seq) FROM tbl_book_rent;

-- SEQ 의 START 값을 적용하는 테이블의 SEQ 최대값보다 크게
-- 설정을 하자
...
/*
위의 코드는 전체 데이터 중에서 대여일과 반납일에 조건을 부여한 후
...
VALUES ( seq_book_rent.NEXTVAL, '2021-04-01','B0012','9791188850266');

INSERT INTO tbl_book_rent(br_seq, br_sdate, br_bcode, br_isbn)
VALUES ( seq_book_rent.NEXTVAL, '2021-04-01','B0012','9791188850389');
