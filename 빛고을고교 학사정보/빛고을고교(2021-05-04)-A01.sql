-- 여기는 kscuser 접속
-- tbl_student, tbl_dept, tbl_sbject, tbl_score TABLE 생성
...
-- 제2정규화 된 테이블에는 통계함수를 적용하여 적용할 수 있다.
...
-- 과목명 칼럼에 국어 문자열이 담겨 있으면 해당 레코드의 점수 칼럼 값을 표시하고
--      그렇지 않으면 (null)을 표시하라
SELECT 학번,
...
-- PIVOT 하여 일반적인 보고서 List 형태로 만드는 형태
...
(
    SELECT 학번,
        ...
        SUM(DECODE(과목명,'데이터베이스',점수)) AS DB,
