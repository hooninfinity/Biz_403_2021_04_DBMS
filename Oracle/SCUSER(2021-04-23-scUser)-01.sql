...
-- INSERT 수행하기
-- 이대로 만들면 항상 CREATE로 작성한 칼럼의 순서를 기억해야 하고
...
-- 한 학생의 점수가 2중, 2중으로 INSERT 되어 엉뚱한 결과가 나타날 수 있다
-- 제약조건 부여
-- 1. 학번은 중복될 수 없고 절대 NULL 값이어서는 안된다.
...
CREATE TABLE tbl_score(
    ...
    PRIMARY KEY (sc_num) -- PRIMARY KEY를 별도로 여러개 등록할때 이쪽에 선언하기도 한다.
);
...
-- SELECT를 사용하여 데이터를 조회하는데 계산하는 칼럼도 있고
-- 자꾸 문법이 복잡해지려고 한다.
...
-- 다른 과목의 점수는 감추고 싶다.
-- 보안적인 측면에서 사용자별로
-- 보여줄 항목, 보이지 않을 항목을 선별하여 VIEW작성해 두면
