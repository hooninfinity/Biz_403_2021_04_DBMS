...
-- USER(사용자 계정)
-- 일반적인 DBMS 소프트웨어에서는 사용자 계정은 DBMS 서버에 접속하기 위한 역할만 수행한다
-- Oracle 에서의 사용자 계정은 데이터 저장소와 연결하는 독특한 역할도 수행한다.
-- A라는 사용자가 생성한, 연결된 TableSpace가 있다면
--      다른 사용자는 TableSpace에 데이터를 추가, 수정, 삭제, 조회하는데 제한을 둘 수 있다.
-- 사용자를 등록할때 사용자만의 TableSpace를 지정하여 줄 수 있다.
