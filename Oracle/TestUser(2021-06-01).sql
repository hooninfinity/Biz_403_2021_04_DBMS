...
SELECT bk_comp AS 출판사코드, COUNT(*) AS 권수
FROM tbl_books
GROUP BY bk_comp;

SELECT bk_comp AS 코드, cp_name AS 출판사명, COUNT(*) 권수
FROM tbl_books B
...
-- tbl_books, tbl_company, tbl_author 세개의 table을 JOIN하여
-- ISBN, 도서명, 출판사명, 출판사대표, 저자, 저자연락처, 출판일 으로
-- 출력되도록 SQL 작성
-- 출판일이 2018년 데이터만
SELECT
    B.bk_isbn AS ISBN,
...
/*
    SUBSTR(문자열 칼럼, 시작위치, 개수) : 오라클전용
    MySQL : LEFT(문자열 칼럼, 앞에서 몇개)
*/
-- 자주 쓸것 같으면 view로 만들어보자
...
그런데 자주 사용할 것 같지 않은 경우 view로 생성하면 아무래도 저장공간을
차지하게 된다.
이럴때 한개의 SQL(SELECT)를 마치 가상의 table처럼 FROM 절에 부착하여 사용가능
*/
SELECT * FROM
(
    SELECT
...
-- FK
-- 두 개 이상의 table을 연결하여 view(조회)를 할때
